...
async fn main() {
    ...
    // Initialise shared telemetry instruments
    let telemetry: TelemetryState = Arc::new(Telemetry::new());
...
async fn todos_index(
    ...
    tel.active_requests.add(-1, &[KeyValue::new("http.route", route)]);
    tel.request_duration.record(elapsed, &[
        KeyValue::new("http.request.method", method),
        KeyValue::new("http.route", route),
...
async fn todos_create(
    ...
    // Flow entry: count every create attempt for throughput SLI
    tel.flow_outcomes.add(1, &[KeyValue::new("outcome", "started")]);
    ...
    // Validation passed (body parsed successfully by Axum extractor)
    tel.validation_outcomes.add(1, &[KeyValue::new("outcome", "passed")]);
...
) -> Result<impl IntoResponse, StatusCode>
{
    ...
    if let Some(completed) = input.completed{
        // When a todo is marked completed, record the flow as successful
        if completed {
            let flow_elapsed = start.elapsed().as_secs_f64();
            tel.flow_outcomes.add(1, &[KeyValue::new("outcome", "success")]);
            tel.flow_duration.record(flow_elapsed, &[KeyValue::new("outcome", "success")]);
        }
